...
function displayDifficulty(value) {
  ...
  if (difficulty === "basic" || difficulty === "easy") return "Basic";
  if (difficulty === "intermediate" || difficulty === "medium")
    return "Intermediate";
...
function DifficultyBadge({ difficulty }) {
  const value = displayDifficulty(difficulty);
...
const [isCsvUploading, setIsCsvUploading] = useState(false);
  ...
  try {
    ...
    rawRows.forEach((item) => {
      ...
      // Case 1: API already returns flat rows
      if (
        item.industry ||
        item.industry_name ||
        item.category ||
      ...
      // Case 2: API returns nested industry → categories → subcategories
    ...
    setHierarchyRows(
      flatRows.filter((row) => row.industry && row.industry !== "-")
    );
  } catch (error) {
    console.error("Hierarchy load error:", error);
    toast.error(error.message || "Failed to load hierarchy filters");
  }
}
...
const examOptions = useMemo(() => {
  return getUniqueOptions(mainQuestions, "exam_id");
}, [mainQuestions]);

const industryOptions = useMemo(() => {
  return getUniqueOptions(hierarchyRows, "industry");
}, [hierarchyRows]);
...
const categoryOptions = useMemo(() => {
  const rows =
    industryFilter === "all"
      ? hierarchyRows
      : hierarchyRows.filter((row) => row.industry === industryFilter);

  return getUniqueOptions(rows, "category").filter((value) => value !== "-");
}, [hierarchyRows, industryFilter]);

const subCategoryOptions = useMemo(() => {
  let rows = hierarchyRows;

  if (industryFilter !== "all") {
    rows = rows.filter((row) => row.industry === industryFilter);
  }
  ...
  return getUniqueOptions(rows, "sub_category").filter((value) => value !== "-");
}, [hierarchyRows, industryFilter, categoryFilter]);
...
const filteredMainQuestions = useMemo(() => {
  ...
  return mainQuestions.filter((question) => {
    ...
    const matchesIndustry =
      industryFilter === "all" || question.industry === industryFilter;
    ...
    const matchesSubCategory =
      ...
      question.sub_category === subCategoryFilter;
...
const totalQuestionsCount =
  questionsPagination?.total || mainQuestions.length;

const totalPages =
  questionsPagination?.totalPages ||
  Math.max(1, Math.ceil(totalQuestionsCount / pageSize));

const paginatedQuestions = mainQuestions;
      ...
      <Card className="rounded-2xl border border-border shadow-sm">
        ...
        {activeTab === "main" && (
          <div className="grid gap-3 xl:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_auto]">
            ...
            >
              <option value="all">Exam ID: All</option>
